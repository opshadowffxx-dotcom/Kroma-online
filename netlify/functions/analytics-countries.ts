...
import { fetchGA4TopCountries } from '../../server/ga4DataService';
...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    ...
    const result = await fetchGA4TopCountries({
    ...
    console.error('[Netlify GA4 Analytics Countries] Error:', err?.message || err);
    return {
      statusCode: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...
        message: err?.message || 'Failed to process GA4 countries request.',
      }),
    };
  }
};
